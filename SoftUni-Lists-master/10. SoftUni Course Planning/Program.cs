    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                if (firstCmd == "Add")
                {
                    string currenLecture = commands[1];
                    if (!lessons.Contains(currenLecture))
                    {
                        lessons.Add(currenLecture);
                    }
                }
                else if (firstCmd == "Insert")
                {
                    ...
                    if (!lessons.Contains(currenLecture))
                    {
                        ...
                        {
                            lessons.Insert(givenIndexFromCmd, currenLecture);
                        }
                    }
                ...
                {
                    string currenLecture = commands[1];
                    lessons.Remove(currenLecture);
                    lessons.Remove($"{currenLecture}-Exercise");
                }
                else if (firstCmd == "Swap")
                {
                    ...
                    {
                        lessons[indexOfLesson1] = lessonOneTitle;
                        lessons[indexOfLesson2] = lessonTwoTitle;
                        if (indexOfLesson1 + 1 < lessons.Count && lessons[indexOfLesson1 + 1] 
                            == $"{lessonOneTitle}-Exercise")
                        {
                        ...
                        if (indexOfLesson2 + 1 < lessons.Count && lessons[indexOfLesson2 + 1]
                          == $"{lessonTwoTitle}-Exercise")
                        {
